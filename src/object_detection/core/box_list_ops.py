...
def clip_to_window(boxlist, window, filter_nonoverlapping=True, scope=None):
    ...
    with tf.name_scope(scope, 'ClipToWindow'):
        ...
        clipped = box_list.BoxList(tf.concat(
...
def prune_outside_window(boxlist, window, scope=None):
    ...
    with tf.name_scope(scope, 'PruneOutsideWindow'):
        ...
        valid_indices = tf.reshape(tf.where(tf.logical_not(
            tf.reduce_any(coordinate_violations, 1))), [-1])
...
def prune_completely_outside_window(boxlist, window, scope=None):
    ...
    with tf.name_scope(scope, 'PruneCompleteleyOutsideWindow'):
        ...
        coordinate_violations = tf.concat(
            [
                tf.greater_equal(
                    y_min, win_y_max), tf.greater_equal(
                    x_min, win_x_max), tf.less_equal(
                    y_max, win_y_min), tf.less_equal(
                        x_max, win_x_min)], 1)
        valid_indices = tf.reshape(tf.where(tf.logical_not(
            tf.reduce_any(coordinate_violations, 1))), [-1])
        return gather(boxlist, valid_indices), valid_indices
...
def sort_by_field(boxlist, field, order=SortOrder.descend, scope=None):
    ...
    with tf.name_scope(scope, 'SortByField'):
        ...
        with tf.control_dependencies([length_assert]):
            # TODO: Remove with tf.device when top_k operation runs correctly
            # on GPU.
