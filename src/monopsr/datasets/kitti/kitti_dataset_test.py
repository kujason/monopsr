...
class KittiDatasetTest(unittest.TestCase):
    ...
    def test_data_loading(self):
        ...
        samples = dataset.get_sample_dict(indices_to_load)

        # Loop through indices
        for i in range(len(indices_to_load)):
            # Check image
            sample_image_input = samples[i].get(constants.SAMPLE_IMAGE_INPUT)
            ...
            # Check labels
            label_boxes_2d = samples[i].get(constants.SAMPLE_LABEL_BOXES_2D)
            self.assertIsNotNone(label_boxes_2d)
            self.assertIsInstance(label_boxes_2d, np.ndarray)
            self.assertIsInstance(label_boxes_2d[0], np.ndarray)

            label_boxes_3d = samples[i].get(constants.SAMPLE_LABEL_BOXES_3D)
            self.assertIsNotNone(label_boxes_3d)
            self.assertIsInstance(label_boxes_3d, np.ndarray)
            self.assertIsInstance(label_boxes_3d[0], np.ndarray)

            label_class_indices = samples[i].get(constants.SAMPLE_LABEL_CLASS_INDICES)
            self.assertIsNotNone(label_class_indices)
            self.assertIsInstance(label_class_indices, np.ndarray)
            self.assertIsInstance(label_class_indices[0], np.int32)
